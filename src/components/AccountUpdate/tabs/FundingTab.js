...
class FundingTab extends Component {
  constructor(props) {
    ...
    this.defaultFundingOptions = [];
  }

  getOptionsToDisplay = () => {
    // Use API options if available, otherwise fall back to defaults (which are now empty)
    const apiOptions = this.props.data.options;
    console.log('🎯 [FundingTab] getOptionsToDisplay: apiOptions =', apiOptions);
    
    if (apiOptions && apiOptions.length > 0) {
      console.log('🎯 [FundingTab] ✅ Using API options:', apiOptions);
      return apiOptions;
    } else {
      console.log('🎯 [FundingTab] ❌ Using empty default options (API options not available or empty)');
      return this.defaultFundingOptions;
    }
  };
